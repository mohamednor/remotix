...
import '../../core/storage/tv_storage.dart';
...
class SamsungTizenDriver implements TvDriver {
  ...
  Future<void> sendCommand(TvCommand command) async {
    ...
    if (command == TvCommand.power && !isConnected) {
      final mac = await TvStorage.loadMac(ipAddress);
      if (mac == null || mac.trim().isEmpty) {
        throw const DriverException('MAC not set (needed for Wake-on-LAN)');
      }
      AppLogger.i('Samsung WOL: Sending magic packet to $mac');
